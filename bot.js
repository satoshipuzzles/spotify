...
// 1) Wire up WebSocket in Node.js
...
// --- Start subscription & event loop ---
(async () => {
  console.log(`▶️  Subscribing for mentions as ${BOT_PK} on ${RELAYS.join(', ')}`);

  // subscribe() takes: [relay URLs], [ filter objects... ]
  const sub = pool.subscribe(
    RELAYS,
    [{ kinds: [1], '#p': [BOT_PK] }]
  );

  // loop over each matching event
  for await (const event of sub) {
    try {
      console.log('▶️  Mention:', event);

      // extract all Spotify track IDs from the content
